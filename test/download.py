...
def terraform_exec(workdir, executable: Path, cmd: list[str], module: str):
    os.makedirs(workdir, exist_ok=True)
    with open(os.path.join(workdir, 'terraform.tf'), 'w') as f:
        f.write(module)

    # Here we go
    result = subprocess.run(
        [str(executable)] + cmd,
        env=os.environ | {'TF_INPUT': 'false', 'TF_CLI_ARGS': '-no-color'},
        capture_output=True,
        cwd=workdir
    )
